...
_Last updated: 2026-06-09 — **Graph write path now LIVE-functional (two live-only defects fixed, verified end-to-end against a real Exchange Online tenant via the gated `GraphDestinationLiveTests`).** (1) MIME import: the folder-scoped `POST .../mailFolders/{id}/messages` silently rejects `text/plain` MIME (`UnableToDeserializePostBody`) — fixed by importing at the **top-level** `POST .../messages` then `move`-ing into the destination folder; source read/unread preserved via `isRead` (imported messages are unavoidably `isDraft=true` per Graph — mitigation deferred). (2) Custom folders never resolved because live Graph reports a top-level folder's `parentFolderId` as the root's **real id**, not the literal `"msgfolderroot"` — fixed in `GraphMailFolderNode.BuildFromGraph` (a parent absent from the complete fetched set IS the root). Both shipped green under WireMock (which validates neither the body nor the live folder shape); the new gated live test closes that blind spot. Graph suite 108 + 2 live skips, solution build 0/0; details in `docs/KNOWN-ISSUES.md`. Prior: **Plan 11 (Reconcile & attachment backfill) COMPLETE — 11 plans now ✅.** A new OPTIONAL `IReconcilableDestination` capability (Graph/Exchange-only; Gmail/IMAP deliberately don't implement it) + a `ReconcileConsumer` diff the source against the **live** destination (metadata only) and per message copy-missing / backfill-only-the-missing-attachments / skip-complete — never duplicating (the dest is scanned live; re-runs write nothing). As a shared by-product the Graph write path now handles large attachments: base64(MIME) ≤ 4 MB keeps the byte-identical single POST, larger messages import a reduced MIME (largest parts stripped) then re-add each part via an attachment upload session (POST <3 MB / `LargeFileUploadTask` 3–150 MB), with an S/MIME guard that never strips a signed/enveloped message. Triggers: `POST /migrations/{id}/reconcile` (+ `Job.Mode` enum/migration + `IJobOrchestrator.EnqueueReconcileAsync`), an `emaigrator reconcile --id [--match metadata|hash]` CLI verb (in-process worker, connector-shaped secret bundle), and a "Reconcile / repair" SPA action on the Results view (typed `reconcile()` wrapper, busy/error states). Bodies/attachments transit memory only — the security userGate proves a real copy+backfill canary run persists 0 body/attachment bytes to any Postgres column, and the functional userGate proves copy/backfill/skip + no-dupes + idempotent re-run; both adversarially verified (break→FAIL→revert). Non-blocking follow-up: strict `--match=hash` ships as an accepted/validated flag only (heavy impl deferred per design §9). Prior: **Wave F (live frontend↔API integration) substantially landed.** The SPA now ships INSIDE the Docker stack (an nginx `web` service serves the production build and reverse-proxies `/api`+`/hubs` to the `api` service → single same-origin, so `docker compose up` brings up the whole app at `http://localhost:3000`). Real auth wired: `/login` page (login/register, register auto-signs-in) + a global 401→/login redirect; new API endpoints `POST /auth/logout` and `GET /providers` (capability list — drives API-authoritative `canBatch`). Per-row dashboard live updates over SignalR (single hub client), error states on every data view (no more infinite skeletons), and the frontend API types reconciled to the real wire shapes (`migrated` not `migratedCount`, nested `ResultsDto`, audit `date`). Two shipping defects fixed: `docker-compose.yml` env keys now match what the apps bind, and the API applies EF migrations for all 3 DbContexts at startup. Verified live in-browser (login→dashboard→logout) + suites green (.NET solution build 0/0, API 78/78; web 95 + Playwright E2E 1 + build exit 0). Then closed the API-side gaps the reconciliation surfaced: the results endpoint now sends `status`/`durationSeconds`/`logDeletesAt` and preflight `GET` sends `scanning` (200 `{scanning:true}` while the background scan runs), with the SPA re-wired (Results shows real status/duration/log-deletion date; StepReview polls on `scanning`). Only `usage`/quota stays absent by design (hosted-billing, open-core boundary). API 81/81, web 102 + E2E 1, builds 0/0. Prior: **Plan 10 (Frontend) COMPLETE — ALL 10 PLANS NOW ✅.** The Vite + React 19 + TS SPA is built and verified against the frozen CONTRACTS §6 wire shapes (MSW mock API + injectable fake SignalR): 15 sub-tasks via subagent-driven-development (implementer + spec + code-quality review each). `npm --prefix web run test -- --run` 81/81; `build` exit 0; Playwright happy-path E2E `1 passed`; `scan:bundle` OK. The security userGate was adversarially verified (5 controls proven to have teeth) and the functional gate (E2E) likewise (break→fail→revert). A real wizard state-staleness bug was found by the E2E and fixed (`WizardShell` re-fetches the draft per step). Live API/SignalR integration is the documented Wave-F follow-up (usage-data wiring, API-authoritative canBatch, cross-origin WS, per-row dashboard live updates). Prior: **Plan 09 (CLI) COMPLETE** — Tasks 12–15 finished on the working engine (Plan 08R). Secret-bundle unification (secrets stored as connector-shaped JSON: `{"password":…}` for IMAP basic, so connect-test/preflight/run all resolve against the real connector); in-process single-node worker wired into `CliHostBuilder` (`AddInfrastructure(registerBus:false)` + `AddEmaigratorWorkers`); `EfMigrationFactory`/`EfMigrationStateReader`/`EfMigrationResetter` + `SchemaMigratorHostedService`. CLI unit 51/51; CLI integration **7/7** via Testcontainers (preflight+run migrates 20, resume idempotent→25, security userGate, functional userGate). An adversarial-verify workflow (4 skeptics + independent reproduction) returned **GO / HIGH confidence**: no secret leak, no message duplication, no false-green gate. Non-blocking follow-up tracked: the engine resume-completion race in `EMaigrator.Workers` (a single `resume` of a finished migration can report Completed before the re-seeded items finish — delayed/misleading status, **no data loss or duplication**; resume is idempotent). **All 10 plans except Frontend (10) are now ✅.**_
...
| 10 | Frontend | 15 | ✅ done | C→F | 2026-06-04 · `npm --prefix web run test -- --run` (81 passed, 31 files: unit + a11y + security) + `npm --prefix web run build` (tsc -b strict, exit 0) + `npm --prefix web run e2e` (Playwright happy-path **1 passed** vs MSW mock + fake SignalR) + `npm --prefix web run scan:bundle` (**bundle-scan OK**, exit 0). Security userGate **adversarially verified** — 5 controls each proven to have teeth (inject→fail→revert): no secret in storage, cookie auth (`credentials:include`, no token Authorization, no storage-reading SignalR accessTokenFactory), XSS-safe escaped rendering (no `dangerouslySetInnerHTML`), no secret-shaped content in dist bundle, CSP meta. a11y: vitest-axe gate (dashboard + wizard step, matcher adversarially proven), keyboard/focus, status icon+label. 15 commits one-per-task. **Wave F live integration landed (2026-06-05):** in-compose SPA via nginx same-origin proxy, auth + logout, `GET /providers`-driven canBatch, per-row live SignalR, error states, and frontend↔API DTO reconciliation. The follow-up API-side gaps were then closed (results `status`/`durationSeconds`/`logDeletesAt`, preflight `scanning`); only hosted-only `usage`/quota stays absent by design. |

| 11 | Reconcile & attachment backfill | 12 | ✅ done | post-F | 2026-06-09 · Suites green: Core matcher + NetArchTest, Gmail 81/81, Graph 93 (+1 opt-in skip), Workers unit 39/39, API 84/84, CLI 55/55, web 105/105 + build exit 0; solution build 0/0. **userGates** via Testcontainers (Postgres/RabbitMQ/Redis), both proven with teeth: security `ReconcileNoBodyPersistence` (a real copy+backfill canary run leaves 0 body/attachment bytes across every Postgres column; forced canary → FAIL → revert) + Infra forbidden-column Theory extended to `jobs`/`mailbox_migrations` with `Job.Mode` allow-listed; functional `ReconcileEndToEnd` (copy-missing / backfill-incomplete / skip-complete, no duplicates, idempotent re-run; ignore-index → duplicate FAIL → revert). |
