...
_Last updated: 2026-06-04 — Plan 09 (CLI) Tasks 1–11/15 done (full CLI surface: root+global opts, migration new, profile loader, secret resolver, output writers, connect/preflight/run/resume/status/report commands, CommandRunner host wiring, parsing guards — 47 unit tests green, Release 0/0). Tasks 12–15 (Docker E2E + security/functional gates) are **blocked**: the worker per-message data-seams (`IMigrationConnectionLookup`, `IMessageRefLister`, `IMessageHydrator`) have NO production implementation — `PendingWorkerSeams` still throws `NotImplementedException` ("Plan 08 supplies the real EF-backed impl"), but Plan 08 never built them (its functional E2E used fakes). Nothing writes `MailboxMigration.Status` to a terminal state either. So no production path can run a real migration end-to-end. **Decision: reopen Plan 08 to build the worker data-seams as a proper tested plan, then finish 09 Tasks 12–15.**_
...
| 08 | API | 15 | ⚠️ reopened | E | 2026-06-03 REST/SignalR/Identity suite green (74) BUT **worker data-seam gap found 2026-06-04**: the real EF-backed `IMigrationConnectionLookup`/`IMessageRefLister`/`IMessageHydrator` the API was meant to supply (per `PendingWorkerSeams` comment) were never built; `PendingWorkerSeams` still throws and nothing writes `MailboxMigration.Status` terminal. No production path runs a real migration. Reopening to build these with tests. |
| 09 | CLI | 15 | 🔵 in progress | E | Tasks 1–11/15 done 2026-06-04 · `dotnet test src/EMaigrator.Cli.Tests -c Release` (47 passed, 0 failed; full CLI surface + parsing guards). Tasks 12–15 (Docker E2E + security/functional gates) blocked on the Plan 08 worker-seam gap above. |
